...
func (a *App) drawPieces(screen *ebiten.Image) {
	...
	for piece := range a.whiteBoard {
		a.op.GeoM.Reset()
		switch piece.kind {
		case Pawn:
			a.op.GeoM.Translate(108.75*float64(piece.currentPosition%8), 106.125*math.Floor(float64(piece.currentPosition/8)))
